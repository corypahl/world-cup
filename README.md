...
Historical morning standings from the opening day onward can be rebuilt from completed group-stage matches with:

```bash
node scripts/reconstruct-standings-history.mjs 2026-06-11
```

The reconstructed snapshots power the leaderboard rank-history chart. The chart displays selected tournament milestone dates that have passed, plus the current day as its final point.

Match recap participant attribution is added deterministically from `participants.json`, not generated by Gemini. Each AI recap is matched back to its source game by `matchIndex`, so the displayed picked-by names remain accurate even if the model changes response order.

The initial June 17 morning baseline was reconstructed with:

```bash
node scripts/reconstruct-standings-baseline.mjs 2026-06-17
```

The script reverses that date's completed group-stage points from each selected team, reconstructs each participant's score and goals-based tiebreaker, and recalculates ranks. The snapshot records the reversed ESPN match IDs for auditing.
...
Rows with equal current points and tiebreaker display the same rank. The Alive column is informational and shows one icon per picked team: green check for clinched, red X for eliminated, and yellow pending for teams still waiting on a Round-of-32 outcome.
